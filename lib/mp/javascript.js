...
module.exports = function (config, isDevelopment, installPluginsAndDeps) {
  ...
  if (!isDevelopment&&CONFIG_JS.lint) {
    Module.rules.push({
      test: REG_EXTTYPE,
      include: [
        Path.join(process.cwd(), config.basic.source, CONFIG_JS.source)
      ],
      enforce: 'pre',
      use: [{
  ...
  if (installPluginsAndDeps) {
    ...
    Shell.cp(Path.join(__dirname,'../../templates/eslintrc.ejs'), Path.join(process.cwd(),'.eslintrc'));
    // copy .babelrc to project directory
    Shell.cp(Path.join(__dirname,'../../templates/babelrc.ejs'), Path.join(process.cwd(),'.babelrc'));
  }
  ...
  if(CONFIG_JS.splitDllModule && !isDevelopment){
    Plugins.push(new Webpack.DllReferencePlugin({
      context: process.cwd(),
      manifest: Path.join(process.cwd(),'./dll/manifest.json'),
    }));
    Plugins.push(new CopyWebpackPlugin([{
      from: Path.join(process.cwd(), `./dll/${config.basic.appname}.dll.js`),
      to: Path.join(process.cwd(), config.basic.output, `${CONFIG_JS.output}/${config.basic.appname}.dll.js`),
      ignore: ['.*']
    }]));
  }
